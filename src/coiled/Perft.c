...
void Perft(int div)
{
	...
	NumeroJugadas = GenerarMovimientos(Todos, ListaMovimiento);
	...
	for (i = 0; i < NumeroJugadas; i++)
	{
		...
		MiniMax(1, &Perft_Divide_Total, &ListaMovimiento[i].Movimiento, &Perft_Divide_Jugada.D_Nodos);
		MovimientoCoordenadas(CUADRADO_ORIGEN(ListaMovimiento[i].Movimiento), CUADRADO_DESTINO(ListaMovimiento[i].Movimiento), CORONACION(ListaMovimiento[i].Movimiento), Perft_Divide_Jugada.D_Jugada);
...
int MiniMax(int depth, _ST_Perft *P_Divide_Total, int *P_Mov, U64 *Jugada)
{
	...
	for (i = 0; i < NumeroJugadas; i++)
	{
		...
		MiniMax(depth + 1, P_Divide_Total, &ListaMovimiento[i].Movimiento, Jugada);
