...
const uint32_t NnueVersion = 0x7AF32F16u;
...
void half_kp_append_active_indices(const Position *pos, const int c,
    IndexList *active)
{
...
void half_kp_append_changed_indices(const Position *pos, const int c,
...
void append_changed_indices(const Position *pos, IndexList removed[2],
...
#ifdef _MSC_VER
weight_t hidden1_weights[32 * 512];
weight_t hidden2_weights[32 * 32];
weight_t output_weights[1 * 32];

int32_t hidden1_biases[32];
int32_t hidden2_biases[32];
#else
weight_t alignas(64) hidden1_weights[32 * 512];
weight_t alignas(64) hidden2_weights[32 * 32];
weight_t alignas(64) output_weights[1 * 32];

int32_t alignas(64) hidden1_biases[32];
int32_t alignas(64) hidden2_biases[32];
#endif
...
#else
int16_t alignas(64) ft_biases[kHalfDimensions];
int16_t alignas(64) ft_weights[kHalfDimensions * FtInDims];
#endif
...
#else
    clipped_t alignas(64) input[FtOutDims];
#endif
...
int nnue_evaluate_pos(Position *pos)
{
  int32_t out_value;
#ifdef _MSC_VER
  mask_t input_mask[FtOutDims / (8 * sizeof(mask_t))];
  mask_t hidden1_mask[8 / sizeof(mask_t)] = { 0 };
#else
  mask_t alignas(8) input_mask[FtOutDims / (8 * sizeof(mask_t))];
  mask_t alignas(8) hidden1_mask[8 / sizeof(mask_t)] = { 0 };
#endif
...
const char *read_hidden_weights(weight_t *w, unsigned dims,
    const char *d)
{
