...
void HistoricoActualizar(int depth, int *ply, int M, int kMate, int* ML, int Nmov);	/* Actualización histórico para el mejor movimiento y posible killer. Indicará histórico bueno */
int MovimientoInquieto(int *M);														/* Es un movimiento que no genera cambio de material */
int ComprobarMovimientoKiller(int *ply, int *M);									/* Indica si el movimiento es un killer o killer mate */
...
int HistoricoValor(int *M);															/* Histórico valor. Para la ordenación de movimientos */
#endif